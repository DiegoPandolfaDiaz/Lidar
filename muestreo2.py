...
serial_lidar.flush()
serial_lidar.write('DX'+ '\n')
time.sleep(0.1)
serial_lidar.flush()
lines = config_file.readlines()
serial_lidar.flushInput()
serial_lidar.write('LR' + lines[1].split(';')[1])
time.sleep(0.1)
status = ""
while(serial_lidar.inWaiting()>0):
	status = serial_lidar.read(9)
	serial_lidar.flushInput()
	print "status : ", status
if(len(status) == 9):
...
line = lines[2]
serial_lidar.flush()
serial_lidar.write('MS' + line.split(';')[1])
time.sleep(1)
status = ""
while(serial_lidar.inWaiting()>0):
	status = serial_lidar.read(9)
...
serial_lidar.write(cmd + "\n")
time.sleep(1)
serial_lidar.reset_input_buffer()
...
################################################
# lectura serial lidar
	output_lidar = ""
	if(serial_lidar.inWaiting()>0):
		data_lidar = serial_lidar.read(7)
		serial_lidar.flushInput()
		output_lidar = data_lidar.encode("hex")
		print "lidar :\t", output_lidar
